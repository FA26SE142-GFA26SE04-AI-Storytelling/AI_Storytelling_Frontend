...
export const QRCodeScannerTab: React.FC<QRCodeScannerTabProps> = ({ onCodeDetected }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-4 rounded-2xl bg-zinc-900/60 border border-zinc-800 text-xs">
      {/* Video stream container */}
      <div className="relative w-full max-w-xs h-48 rounded-2xl bg-black border border-zinc-800 overflow-hidden flex items-center justify-center">
        {isCameraActive ? (
          <>
            <video
              ref={videoRef}
      ...
      {cameraError && (
        <div className="w-full p-2.5 rounded-xl bg-rose-950/60 border border-rose-500/40 text-rose-300 text-[11px] flex items-center gap-2">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 w-full pt-1">
        {!isCameraActive ? (
          ...
          <button
            type="button"
            onClick={stopCamera}
            className="px-3.5 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
          >
            <span>Tắt Camera</span>
          </button>
        )}

        <label className="px-3.5 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-zinc-200 font-bold flex items-center gap-1.5 transition-colors cursor-pointer">
