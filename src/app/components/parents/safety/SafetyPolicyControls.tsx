...
export const SafetyPolicyControls: React.FC<SafetyPolicyControlsProps> = ({
  ...
  return (
    <div className="space-y-3.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-white">Trạng Thái Nghiệp Vụ:</span>
              <span
      ...
      <div className="laptop-tab-content-row p-3.5 rounded-2xl bg-zinc-900/80 border border-zinc-800 flex flex-col gap-2.5">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-zinc-200 flex items-center gap-1.5">
            <FileText className="w-4 h-4 text-sky-400" />
            Giới Hạn Độ Dài Tối Đa Của Truyện (Max Story Length)
          </span>
          <span className="text-xs font-black text-sky-400 bg-sky-950/60 border border-sky-500/30 px-2 py-0.5 rounded-lg">
          ...
          {[
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {safetyMaxStoryLength === item.len && <Check className="w-3.5 h-3.5 text-sky-400" />}
              </div>
              <span className="text-[9px] font-normal text-zinc-400">{item.desc}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              {safetyApprovalMode === 'AlwaysManual' && <Check className="w-3.5 h-3.5 text-emerald-400" />}
            </div>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              {safetyApprovalMode === 'AutoPublishOnThreshold' && <Check className="w-3.5 h-3.5 text-emerald-400" />}
            </div>
            <span className="text-[10px] text-zinc-400">
      ...
      <div className="laptop-tab-content-row p-3.5 rounded-2xl bg-zinc-900/80 border border-zinc-800 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
        ...
        <button
          ...
          className={`w-12 h-6 rounded-full transition-colors relative cursor-pointer shrink-0 ${
            safetyParentalGate ? 'bg-purple-600 shadow-md shadow-purple-500/20' : 'bg-zinc-700'
          }`}
        >
          <div
      ...
      <div className="laptop-tab-content-row p-3.5 rounded-2xl bg-zinc-900/80 border border-zinc-800 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xs font-bold text-zinc-200 flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-zinc-400 block mt-0.5">
              <strong className="text-rose-400">Quy tắc cốt lõi:</strong> Nội dung bị cấm (Blocked) sẽ tuyệt đối{' '}
              <strong className="text-amber-300">override mọi yêu cầu sáng tạo</strong> của câu chuyện.
            </span>
          </div>
          {isLoadingCategories && <RefreshCw className="w-3.5 h-3.5 text-zinc-400 animate-spin" />}
        </div>
        ...
        <div className="space-y-2">
          {contentCategories.map((category) => {
            ...
            return (
              <div
                key={category.id}
                className="p-2.5 rounded-xl bg-zinc-950/70 border border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2"
              >
                <div className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-zinc-500" />
                  <span className="text-xs font-semibold text-zinc-200">{category.displayName}</span>
      ...
      <div className="laptop-tab-content-row p-3.5 rounded-2xl bg-zinc-900/80 border border-zinc-800 flex flex-col gap-2">
        <label className="flex items-start gap-2.5 cursor-pointer">
          <input
            ...
            className="mt-0.5 w-4 h-4 rounded border-zinc-700 bg-zinc-950 text-indigo-500 focus:ring-indigo-400 accent-indigo-500 cursor-pointer"
          />
