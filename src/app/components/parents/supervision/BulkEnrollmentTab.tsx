...
export const BulkEnrollmentTab: React.FC<BulkEnrollmentTabProps> = ({ onSwitchToSingle }) => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-200">
      {/* Intro Banner */}
      <div className="p-5 rounded-2xl bg-gradient-to-r from-purple-950/60 via-zinc-900 to-indigo-950/60 border border-purple-500/30 text-xs space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-zinc-900 border border-zinc-700 hover:border-indigo-400 text-zinc-300 text-xs font-bold flex items-center gap-1.5 self-start sm:self-auto transition-colors cursor-pointer"
          >
      ...
      <div className="p-6 rounded-2xl bg-zinc-900/80 border border-dashed border-zinc-700 flex flex-col items-center justify-center text-center gap-3">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-zinc-900 border border-zinc-700 hover:border-zinc-500 text-zinc-300 font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
          >
        ...
        {selectedFileName && (
          <div className="mt-2 p-2 px-3 rounded-xl bg-purple-950/50 border border-purple-500/40 text-purple-200 text-xs flex items-center gap-2">
            <FileSpreadsheet className="w-4 h-4 text-purple-400" />
            <span>Đã chọn: <strong>{selectedFileName}</strong></span>
            <span className="text-[10px] text-zinc-400 italic">(Đang chờ kích hoạt backend Luồng 7)</span>
          </div>
        )}

        <div className="pt-3 border-t border-zinc-800/80 w-full max-w-lg text-[11px] text-zinc-500 flex items-center justify-center gap-2">
