...
export const VerifyEmailForm: React.FC<VerifyEmailFormProps> = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      <div className="auth-form-field p-3 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 text-xs flex flex-col gap-1">
        <span className="font-bold text-indigo-600 dark:text-indigo-300 flex items-center gap-1.5">
          <Key className="w-4 h-4 text-amber-500 dark:text-amber-300" />
      ...
      <div className="auth-form-field flex flex-col gap-1">
        <label className="text-[11px] font-bold text-tod-text">Mã Xác Thực (Token từ Gmail) *</label>
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-tod-card border border-tod-border text-xs text-tod-text placeholder-tod-text-muted focus:outline-none focus:border-sky-500 transition-colors font-mono"
          />
        </div>
      </div>
